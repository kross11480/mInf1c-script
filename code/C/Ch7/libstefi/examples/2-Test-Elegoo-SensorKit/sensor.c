...
void mpu6500_write_reg(uint8_t reg, uint8_t data) {
    gpio_write(A4, LOW);
    spi_transfer(PERIPHERAL_ID_SPI1, reg & 0x7F);
...
uint8_t mpu6500_read_reg(uint8_t reg) {
    uint8_t data;
    gpio_write(A4, LOW);
    spi_transfer(PERIPHERAL_ID_SPI1, reg | 0x80);
...
int16_t mpu6500_read_val(uint8_t reg) {
    ...
    gpio_write(A4, LOW);
    spi_transfer(PERIPHERAL_ID_SPI1, reg | 0x80);
    hi = spi_transfer(PERIPHERAL_ID_SPI1, 0xFF);
    lo = spi_transfer(PERIPHERAL_ID_SPI1, 0xFF);
    gpio_write(A4, HIGH);
    return ((hi << 8) | lo);
}
